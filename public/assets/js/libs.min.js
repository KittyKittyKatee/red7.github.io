...
myMap.geoObjects.add(myPlacemark);}});// .footer scripts goes here
$(function(){//мобильное меню
...
var fixedMenu=$(".header__fixed");$(window).scroll(function(){$(this).scrollTop()>window.innerHeight*2?fixedMenu.fadeIn():fixedMenu.fadeOut();});});$(function(){var elem=$('.interiors-slider__main');var arrContent=['Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.','Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.'];var slider=elem.find('.slider__imgs');var prevBtn=elem.find('.slider__arrow--prev');var nextBtn=elem.find('.slider__arrow--next');initSlider(elem,slider,prevBtn,nextBtn,arrContent);});$(function(){var elem=$('.interiors-slider__main');var arrContent=['Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.','Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.'];var slider=elem.find('.slider__imgs');var prevBtn=elem.find('.slider__arrow--prev');var nextBtn=elem.find('.slider__arrow--next');initSlider(elem,slider,prevBtn,nextBtn,arrContent);});$(function(){var arr=[];var name=null;var tel=null;$('.offer-submit').click(function(){var nameInput=$('[name="name-2"]');var telInput=$('[name="tel-2"]');name=nameInput.val().trim();tel=telInput.val().trim();if(arr.includes('name')){if(tel.length<12){telInput.addClass('error');telInput.one('focus',function(){telInput.removeClass('error');});}else{tel=telInput.val();$('.offer__input-wrap').css('transform','translateX(-980px)');$('.offer-submit').css('display','none');}}else{if(name.length<3){nameInput.addClass('error');nameInput.one('focus',function(){nameInput.removeClass('error');});}else{$('.offer__input-wrap').css('transform','translateX(-490px)');arr.push('name');}}});$('.offer__input-wrap').submit(function(e){e.preventDefault();var data=$('.offer__input-wrap').serialize();console.log(data);$('.offer__input-wrap').css('display','none');$('.offer__input-slider').find('span').css('display','block');$('.offer__count').css('display','none');$('.offer__agree').css('display','none');});});$(function(){$("#tel").mask("+7(999) 999-9999");$("#tel-2").mask("+7(999) 999-9999");var ajaxSend=function ajaxSend(url,formData){fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)}).then(function(response){return alert('Успешно отправлено');})["catch"](function(error){return console.error(error);});};$('.modal__form').submit(function(e){e.preventDefault();var name=$('[name="name"]');name.val(name.val().trim());var tel=$('[name="tel"]');if(name.val().length<3){name.addClass('error');name.one('focus',function(e){name.removeClass('error');});return false;}else if(tel.val().length<12){tel.addClass('error');tel.one('focus',function(){tel.removeClass('error');});return false;}var form=$('#modal__form')[0];var formData=new FormData(form);formData=Object.fromEntries(formData);var url='http://localhost:3000/request';ajaxSend(url,formData);form.reset();// $.ajax({
