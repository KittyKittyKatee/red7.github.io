...
_typeof(global)==="object"?global:(typeof window==="undefined"?"undefined":_typeof(window))==="object"?window:(typeof self==="undefined"?"undefined":_typeof(self))==="object"?self:this);}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{});},{}]},{},[1]);$(function(){var firstSlider=$('.apartments__slides[id = "tab-1"]').find('.apartments__slider-wrap');var prev=$('.apartments__arrow--prev');var next=$('.apartments__arrow--next');firstSlider.slick();prev.on('click',function(){firstSlider.slick('slickNext');});next.on('click',function(){firstSlider.slick('slickNext');});$('.slick-arrow').css('display','none');var tab=$('.tabs');tab.click(changeTab);var initSlider=['tab-1'];function changeTab(e){var id=e.target.id;$('.tabs__tab').removeClass('active');$('.apartments__slides').removeClass('active');$(e.target).toggleClass('active');var content=$('.apartments__slides#'+id);content.toggleClass('active');if(!initSlider.includes(id)){initSlider.push(id);var slider=$('.apartments__slides[id = "'+id+'"]').find('.apartments__slider-wrap');slider.slick();prev.on('click',function(){slider.slick('slickNext');});next.on('click',function(){slider.slick('slickNext');});$('.slick-arrow').css('display','none');}}});$(function(){var elem=$('.benefits__slider');var arrContent=['RED7 - новые «Красные ворота», открывающие дорогу к центру Москвы.','Четыре уровня, ступенчатые своды, переплетение плоскостей - знаковые детали проекта.','Через стеклянную крышу открывается завораживающий вид на возвышающийся фасад дома.','Сочетание естественного и искусственного освещения создает театральную игру света и теней.'];var slider=elem.find('.slider__imgs');var prevBtn=elem.find('.slider__arrow--prev');var nextBtn=elem.find('.slider__arrow--next');initSlider(elem,slider,prevBtn,nextBtn,arrContent);});function initSlider(elem,slider,prevBtn,nextBtn,arrContent){slider.slick({prevArrow:prevBtn,nextArrow:nextBtn,speed:900});var current=elem.find('.slider__counter--large');var scoreSlides=elem.find('.slider__counter--small');var content=elem.find('p');scoreSlides.text(' /'+slider.slick("getSlick").slideCount);current.text(slider.slick('slickCurrentSlide')<10?'0'+(slider.slick('slickCurrentSlide')+1):slider.slick('slickCurrentSlide'));content.text(arrContent[slider.slick('slickCurrentSlide')]);slider.on("afterChange",function(event,slick,currentSlide,nextSlide){current.text(currentSlide<10?'0'+(currentSlide+1):currentSlide+1);content.text(arrContent[currentSlide]);});}$(function(){var firstSlide=$('.banner__slider').children().first();firstSlide.next().css({'z-index':'-1','opacity':'0'});var currentSlide=firstSlide;setInterval(function(){if(!currentSlide.length){currentSlide.css({'z-index':'1','opacity':'1'});}currentSlide.css({'z-index':'-1','opacity':'0'});currentSlide.next().css({'z-index':'1','opacity':'1'});if(!currentSlide.next().length){currentSlide=firstSlide;currentSlide.css({'z-index':'1','opacity':'1'});}else{currentSlide=currentSlide.next();}},7000);});// .footer scripts goes here
$(function(){ymaps.ready(init);function init(){var myMap=new ymaps.Map("map",{// Координаты центра карты.
// Порядок по умолчнию: «широта, долгота».
center:[55.772168,37.648456],// Уровень масштабирования. Допустимые значения:
zoom:12,// Элементы управления
...
]});var myPlacemark=new ymaps.Placemark([55.772168,37.648456],{// Хинт показывается при наведении мышкой на иконку метки.
hintContent:'Москва, ул. Маши Порываевой, 34, БЦ Домников, этаж 10'});// После того как метка была создана, добавляем её на карту.
myMap.geoObjects.add(myPlacemark);}});$(function(){//мобильное меню
var burger=$('.header__burger');var menu=$('.header__main').find('nav');var menuItem=menu.find('.header__link');function openMenu(){burger.toggleClass('close');if(menu.css('display')=='none'){menu.fadeIn();}else{menu.fadeOut();}}burger.click(openMenu);menu.click(menuItem,openMenu);//плавная прокрутка
var link=$(".header__link");link.click(scrollToBlock);function scrollToBlock(){var id=$(this).attr("data-link");var target=$("#"+id).offset().top;$("html, body").animate({scrollTop:target},1300);}//фиксированное меню
var fixedMenu=$(".header__fixed");$(window).scroll(function(){$(this).scrollTop()>window.innerHeight*2?fixedMenu.fadeIn():fixedMenu.fadeOut();});});$(function(){var elem=$('.interiors-slider__main');var arrContent=['Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.','Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.'];var slider=elem.find('.slider__imgs');var prevBtn=elem.find('.slider__arrow--prev');var nextBtn=elem.find('.slider__arrow--next');initSlider(elem,slider,prevBtn,nextBtn,arrContent);});$(function(){var elem=$('.interiors-slider__main');var arrContent=['Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.','Интерьеры. Описание в несколько слов. Краткое.','Еще одно очень красноречивое описание интерьеров.'];var slider=elem.find('.slider__imgs');var prevBtn=elem.find('.slider__arrow--prev');var nextBtn=elem.find('.slider__arrow--next');initSlider(elem,slider,prevBtn,nextBtn,arrContent);});$(function(){$("#tel").mask("+7(999) 999-9999");$("#tel-2").mask("+7(999) 999-9999");var ajaxSend=function ajaxSend(url,formData){fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)}).then(function(response){return alert('Успешно отправлено');})["catch"](function(error){return console.error(error);});};$('.modal__form').submit(function(e){e.preventDefault();var name=$('[name="name"]');name.val(name.val().trim());var tel=$('[name="tel"]');if(name.val().length<3){name.addClass('error');name.one('focus',function(e){name.removeClass('error');});return false;}else if(tel.val().length<12){tel.addClass('error');tel.one('focus',function(){tel.removeClass('error');});return false;}var form=$('#modal__form')[0];var formData=new FormData(form);formData=Object.fromEntries(formData);var url='http://localhost:3000/request';ajaxSend(url,formData);form.reset();// $.ajax({
...
//     console.log('окей')
// })
});var modalContent=[{id:1,title:'Запишитесь на просмотр',text:'Заполните форму и мы с Вами свяжемся',btnText:'Записаться на просмотр'},{id:2,title:'Закажите звонок',text:'Наш менеджер свяжется с вами в ближайшее время',btnText:'Позвоните мне'},{id:3,title:'Получить персональное предложение',text:'Наш менеджер свяжется с вами в ближайшее время',btnText:'Получить предложение'}];function openModal(id){var content=$.grep(modalContent,function(item){return id==item.id;})[0];$('.modal__title').text(content.title);$('.modal__text').text(content.text);$('.modal__btn').text(content.btnText);$('.modal').fadeIn();}$('.banner__btn').click(function(){openModal(1);});$('.header__button-call').click(function(){openModal(2);});$('.apartments__btn').click(function(){openModal(3);});$('.modal__close').click(function(){$('.modal').fadeOut();$('input').val('');});});$(function(){var arr=[];var name=null;var tel=null;$('.offer-submit').click(function(){var nameInput=$('[name="name-2"]');var telInput=$('[name="tel-2"]');name=nameInput.val().trim();tel=telInput.val().trim();if(arr.includes('name')){if(tel.length<12){telInput.addClass('error');telInput.one('focus',function(){telInput.removeClass('error');});}else{tel=telInput.val();$('.offer__input-wrap').css('transform','translateX(-980px)');$('.offer-submit').css('display','none');}}else{if(name.length<3){nameInput.addClass('error');nameInput.one('focus',function(){nameInput.removeClass('error');});}else{$('.offer__input-wrap').css('transform','translateX(-490px)');arr.push('name');}}});$('.offer__input-wrap').submit(function(e){e.preventDefault();var data=$('.offer__input-wrap').serialize();console.log(data);$('.offer__input-wrap').css('display','none');$('.offer__input-slider').find('span').css('display','block');$('.offer__count').css('display','none');$('.offer__agree').css('display','none');});});"use strict";$(function(){ymaps.ready(init);function init(){var myMap=new ymaps.Map("map",{// Координаты центра карты.
// Порядок по умолчнию: «широта, долгота».
center:[55.772168,37.648456],// Уровень масштабирования. Допустимые значения:
zoom:12,// Элементы управления
controls:['zoomControl',// Ползунок масштаба
...
hintContent:'Москва, ул. Маши Порываевой, 34, БЦ Домников, этаж 10'});// После того как метка была создана, добавляем её на карту.
myMap.geoObjects.add(myPlacemark);}});"use strict";$(function(){$("#tel").mask("+7(999) 999-9999");$("#tel-2").mask("+7(999) 999-9999");var ajaxSend=function ajaxSend(url,formData){fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(formData)}).then(function(response){return alert('Успешно отправлено');})["catch"](function(error){return console.error(error);});};$('.modal__form').submit(function(e){e.preventDefault();var name=$('[name="name"]');name.val(name.val().trim());var tel=$('[name="tel"]');if(name.val().length<3){name.addClass('error');name.one('focus',function(e){name.removeClass('error');});return false;}else if(tel.val().length<12){tel.addClass('error');tel.one('focus',function(){tel.removeClass('error');});return false;}var form=$('#modal__form')[0];var formData=new FormData(form);formData=Object.fromEntries(formData);var url='http://localhost:3000/request';ajaxSend(url,formData);form.reset();// $.ajax({
//     url: 'http://localhost:3000/request',
//     method: 'POST',
//     data: formData,
// }).done(function(data){
//     console.log('окей')
// })
});var modalContent=[{id:1,title:'Запишитесь на просмотр',text:'Заполните форму и мы с Вами свяжемся',btnText:'Записаться на просмотр'},{id:2,title:'Закажите звонок',text:'Наш менеджер свяжется с вами в ближайшее время',btnText:'Позвоните мне'},{id:3,title:'Получить персональное предложение',text:'Наш менеджер свяжется с вами в ближайшее время',btnText:'Получить предложение'}];function openModal(id){var content=$.grep(modalContent,function(item){return id==item.id;})[0];$('.modal__title').text(content.title);$('.modal__text').text(content.text);$('.modal__btn').text(content.btnText);$('.modal').fadeIn();}$('.banner__btn').click(function(){openModal(1);});$('.header__button-call').click(function(){openModal(2);});$('.apartments__btn').click(function(){openModal(3);});$('.modal__close').click(function(){$('.modal').fadeOut();$('input').val('');});});/*!
 * Chart.js
 * http://chartjs.org/
 * Version: 2.7.0
 *
